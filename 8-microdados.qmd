...
Com os dados em mãos, podemos partir então para o processo de análise. E isto começa, antes de mais nada, com a definição do plano amostral do estudo.

### Declarando o plano amostral

Para que qualquer estimativa produzida com os microdados seja estatisticamente válida, é preciso declarar ao `srvyr` o tipo de amostragem realizada com a função `as_survey_design`^[No caso do pacote `survey`, essa função é a `svydesign()`]. Sem essa declaração, as estatísticas descritivas obtidas a partir destes dados ignorariam a estratificação e os pesos calibrados, produzindo erros-padrão incorretos e, consequentemente, intervalos de confiança inadequados.
...
Neste caso, o que a função `as_survey_design()` necessita é que forneçamos uma coluna na base de dados `md_dom_bh` com o total de unidades correspondente aos estratos definido em `strata`, ou seja, a quantidade de domicílios total da área de ponderação (no nosso caso.) Uma maneira de obtermos essa informação é somar os valores dos pesos `V0010` em cada área de ponderação. Lembre-se que a ideia do peso de uma unidade amostral é exatamente indicar quantas unidades ela representa em seu estrato. Logo somar os pesos de todas as unidades dentro da área de ponderação será uma aproximação do valor total de domicílios nesta área.

Para tanto, primeiro produzimos uma tabela sumarizada retornando o total de domicílios de cada área de ponderação. Na sequência, com o operador `left_join`, trazemos essa informação de população para cada linha de `md_dom_bh`, informando `code_weighting` como chave. O resultado é uma nova coluna em `md_dom_bh`, que traz o valor correspondente do total de domicílios em cada área de ponderação definida na coluna `code_weighting`, conforme o código abaixo:
