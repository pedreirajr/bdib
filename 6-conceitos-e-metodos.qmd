...
O Censo possui dois objetivos principais, de contar todos os habitantes do território nacional e identificar as condições em que vivem. O resultado é um retrato estatístico de alta resolução do país inteiro, com um detalhamento espacial que nenhuma outra pesquisa periódica consegue oferecer.

O primeiro recenseamento do território brasileiro foi realizado em 1872, no Período Imperial. A partir de 1890, o Censo passou a ter periodicidade decenal, sendo esta frequência formalmente estabelecida pela Lei nº 8.184/1991 [@lei8184]. No século XX, embora as edições previstas para 1910 e 1930 não tenham sido realizadas,  os Censos de 1940, 1950, 1960, 1970, 1980 e 1991 foram realizados com sucesso. Já no século XXI, tivemos as edições de 2000, 2010 e 2022. Este último intervalo maior de 12 anos ocorreu em razão e de dificuldades no planejamento e financiamento da pesquisa, agravadas ainda pela pandemia da COVID-19, o que inviabilizou sua realização no ano previsto de 2020.
...
A realização de um censo demográfico envolve desafios logísticos e metodológicos consideráveis. Um dos principais é a impossibilidade de garantir cobertura total da população. Nas operações censitárias, dois tipos de erro de cobertura podem ocorrer, a *omissão*, quando pessoas que deveriam ter sido recenseadas não foram, e a *inclusão indevida*, quando pessoas são contadas quando não deveriam ter sido. A diferença entre ambos é chamada de *erro líquido de enumeração*, a medida mais usada para avaliar a qualidade da cobertura.
...
A partir dessa compreensão introdutória, é hora de apresentarmos os principais métodos de coleta desta pesquisa e seus respectivos produtos, cujo conteúdo será explorado de forma mais detalhada nos próximos capítulos.

### Os Instrumentos de Coleta do Censo

O **setor censitário** é a menor porção territorial em que o Brasil é dividido para fins de coleta estatística. Trata-se de uma área contínua cujo dimensionamento leva em conta a extensão geográfica e a quantidade de domicílios ou estabelecimentos agropecuários nela presentes [@ibge_quageo]. Nas áreas urbanas, os setores tendem a ser menores e mais densamente ocupados. Nas rurais, abrangem extensões maiores com população dispersa. Conforme mencionado anteriormente, para o Censo de 2022, o território brasileiro foi dividido em aproximadamente **450 mil setores censitários**, quase 135 mil a mais que a edição de 2010.

Cada recenseador é designado para cobrir exatamente um setor, dentro do qual seu percurso é planejado, de modo que sejam listados todos os endereços e aplicados os instrumentos de coleta. Dentro de cada setor, a operação de campo segue uma sequência definida. Em uma primeira etapa, denominada **arrolamento**, o recenseador percorre as faces de quadra do setor para listar os endereços existentes, registrar as coordenadas geográficas de cada um e levantar as condições de infraestrutura do entorno. Somente após concluir esse reconhecimento inicial o recenseador dá início à aplicação dos questionários domiciliares. Essa sequência organiza o conjunto de instrumentos de coleta do Censo, cada um com finalidade, cobertura e método próprios.

#### Questionários Básico e da Amostra

Os dois questionários domiciliares do Censo compartilham uma mesma **data de referência**, que define quem é recenseável. Para o Censo de 2022, esta data foi a do dia 31 de julho de 2022. Qualquer pessoa que residia em um domicílio brasileiro nesse momento é, por definição, parte da população recenseada. Moradores temporariamente ausentes na data da visita do recenseador são registrados normalmente, desde que o domicílio seja seu local habitual de residência.

Existem dois tipos de questionário aplicados, neste caso, denominados de Questionário Básico (ou do Universo) ou Questionário da Amostra. Os itens do **Questionário Básico** são respondidos por todos os domicílios do país e investigam as características fundamentais dos moradores e do domicílio. Este questionário possuía 34 itens no Censo de 2010, que foram reduzidos para 26 questões no Censo de 2022. Os resultados relativos a esta coleta são divulgados em forma de agregados (totais, médias e variâncias), no nível do setor censitário. 

Já o **Questionário da Amostra** é aplicado em uma fração dos domicílios, selecionados de forma aleatória em cada setor censtiário. Inclui todos os quesitos do Básico e acrescenta temas adicionais, como migração, fecundidade, trabalho, rendimento e deslocamento. No Censo de 2010 o questionário tinha 102 quesitos e, em 2022, este número foi de 103. A fração amostral varia com o porte do município: nos menores, chega a 50% dos domicílios, ao passo que nos maiores, pode ser inferior a 10%. Os resultados são divulgados por meio de microdados (respostas individuais), cuja correspondência geográfica ocorre no nível da área de ponderação, recorte que agrega setores censitários em número suficiente de domicílios para garantir precisão estatística e preservar o anonimato dos respondentes. Ambos os questionários são aplicados em entrevista presencial pelo recenseador, com uso de *tablet* dotado do aplicativo de coleta do IBGE.

#### Cadastro Nacional de Endereços para Fins Estatísticos (CNEFE)

O CNEFE é o instrumento de cadastramento georreferenciado de todos os endereços visitados pelo recenseador durante a operação censitária. Enquanto os questionários coletam informações sobre quem mora nos domicílios, o CNEFE registra onde esses domicílios estão. Para cada endereço, o recenseador captura as coordenadas geográficas com o receptor GPS/GNSS integrado ao *tablet*, além de informações sobre a espécie do endereço (se domicílio particular, estabelecimento de saúde ou de ensino, por exemplo) e outras características do logradouro. No Censo de 2022 foram cadastrados 106,8 milhões de endereços com coordenadas geográficas [@cnefe_metodologia2024]. 
...
O trajeto dos recenseadores é o registro georreferenciado automático das rotas percorridas durante a operação de campo. O *tablet* utilizado pelo recenseador captura continuamente a posição GPS/GNSS ao longo do percurso pelo setor, gerando uma trilha que documenta o caminho percorrido e os endereços visitados. O produto resultante permite ao IBGE monitorar a cobertura da coleta em tempo real, identificar setores com lacunas de cobertura e, após a operação, disponibilizar para o público um conjunto de dados que permite visualizar os trajetos de todos os recenseadores no país.

### Os Produtos do Censo

O Censo Demográfico produz um conjunto diversificado de produtos de dados a partir dos instrumentos de coleta apresentados anteriormente, cada um com características distintas de resolução espacial, riqueza temática e forma de acesso. Os principais produtos divulgados pelo IBGE são:

- **Agregados por setor censitário:** estatísticas tabuladas para cada um dos 450 mil setores censitários do país, a partir das respostas do Questionário Básico (universo). Cobrem características dos domicílios e dos moradores. 
