...
Este capítulo é uma introdução ao tema. Há livros e disciplinas inteiras dedicados a geoprocessamento utilizando R, a exemplo de [*Geocomputation with R*](https://r.geocompx.org/) [@lovelace_geocomputation_2025] e [*Spatial Statistics for Data Science*](https://www.paulamoraga.com/book-spatial/) [@moraga_spatial_2023]. Temos um objetivo mais modesto por aqui, que é o de consolidar os requisitos teóricos mínimos para trabalharmos com os dados geográficos das bases de dados brasileiras que ocupam os demais capítulos.
...
#| echo: false
#| fig-cap: "Dois sistemas de referência de coordenadas aplicados ao Brasil: geográfico em SIRGAS 2000 (acima), com eixos em graus de latitude e longitude; e projetado em SIRGAS 2000 / Policônico (abaixo), com eixos em metros."
#| fig-width: 7
#| fig-height: 9
#| cache: true
#| message: false
...
p_proj <- uf_br |>
  ...
  ggplot() +
  geom_sf(fill = fill_color, color = border_color, linewidth = 0.15) +
  coord_sf(datum = sf::st_crs(5880)) +
  labs(title = "SRC Projetado — SIRGAS 2000 / Policônico (EPSG 5880)") +
  map_theme
...
#| fig-cap: "Buffer de 200 metros ao redor da linha do metrô de Salvador."
bff_linha <- st_buffer(lin_metro_utm, dist = 200)

ggplot() +
  geom_sf(data = bairros, fill = "gray92", color = "gray70", linewidth = 0.2) +
  geom_sf(data = bff_linha,    fill = "#f0500f", alpha = 0.30, color = NA) +
  geom_sf(data = lin_metro_utm, color = "darkred", linewidth = 1) +
  theme_minimal()
```
