...
Por exemplo, o fuso que cobre o Rio de Janeiro abrange as longitudes de −48° a −42°. Tomando a longitude −42°, a operação fica [180 + (−42)] / 6 = 138 / 6 = 23. Como o Rio de Janeiro fica ao sul do Equador, o fuso é 23 S. Para Salvador, em vez de estimar a longitude de cabeça, extraímos essa informação diretamente dos dados. A função `st_bbox()` retorna o *bounding box* de um objeto `sf`, um retângulo definido por quatro valores: `xmin` e `xmax` (longitudes extremas) e `ymin` e `ymax` (latitudes extremas). O trecho abaixo extrai o bounding box dos bairros de Salvador:
...
Deste modo, temos os seguintes resultados para cada coordenada:
```{r}
cat("Fuso (xmin):", fuso_oeste, hemi_min, "\n")
cat("Fuso (xmax):", fuso_leste, hemi_max, "\n")
cat("Hemisfério (ymin):", hemi_min,       "\n")
cat("Hemisfério (ymax):", hemi_max,       "\n")
```
